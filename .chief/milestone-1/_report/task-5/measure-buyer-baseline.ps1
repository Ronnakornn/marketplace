...
function Measure-BrowserDump($label, $url, $browserExe) {
  if (-not $browserExe) {
    return [ordered]@{
      label = $label
      url = $url
  ...
  $tempProfile = Join-Path $env:TEMP ("buyer-baseline-browser-" + [guid]::NewGuid().ToString("N"))
  New-Item -ItemType Directory -Force -Path $tempProfile | Out-Null
  $stdoutPath = Join-Path $env:TEMP ("buyer-baseline-browser-" + [guid]::NewGuid().ToString("N") + ".html")
  $stderrPath = Join-Path $env:TEMP ("buyer-baseline-browser-" + [guid]::NewGuid().ToString("N") + ".err")
  ...
  try {
    ...
    return [ordered]@{
      ...
      hasMarketplaceText = $normalized -match "Marketplace|Shopee|Flash|Search"
...
try {
  ...
  $results = [ordered]@{
    ...
    notes = @(
      "Local development-mode baseline before buyer-entry optimization.",
      "HTTP timings use curl's transfer buckets and include Next.js route responses plus representative public API calls used by the client pages.",
      "Browser observations use a local headless Edge/Chrome DOM dump when available. This is visible DOM evidence, not a full Playwright trace or precise first-paint metric.",
      "No production app or server code was modified for this measurement."
    )
    httpRequests = @(
    ...
    browserObservations = @(
      (Measure-BrowserDump "home /th" $homeUrl $browserExe),
      (Measure-BrowserDump "search /th/search?q=phone" $searchUrl $browserExe)
    )
  }
