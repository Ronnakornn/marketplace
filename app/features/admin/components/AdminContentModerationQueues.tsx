...
const QUEUES: QueueKey[] = ["reviews", "reports", "questions", "answers"];
...
function FilterSelect(props: {
  ...
  return (
    <Select
      ...
      onValueChange={(value) => props.onChange(value === "ALL" ? "" : value)}
    >
      ...
      <SelectContent>
        ...
        {props.options.map((option) => (
          <SelectItem key={option} value={option}>
            {t(`admin.statuses.${option}` as Parameters<typeof t>[0]) ===
            `admin.statuses.${option}`
              ? option.replaceAll("_", " ")
              : t(`admin.statuses.${option}` as Parameters<typeof t>[0])}
          </SelectItem>
        ))}
  ...
  return (
    <TableRow className="border-white/8 hover:bg-transparent">
      <TableCell colSpan={6} className="h-32 text-center text-slate-400">
        {t("admin.contentModeration.empty")}
      </TableCell>
    </TableRow>
  );
}
...
export function AdminContentModerationQueues() {
  ...
  const [page, setPage] = useState<Record<QueueKey, number>>({
    reviews: 1,
    reports: 1,
    questions: 1,
    answers: 1,
  });
  const [status, setStatus] = useState<Record<QueueKey, string>>({
    reviews: "PENDING",
    reports: "OPEN",
    questions: "PENDING",
    answers: "PENDING",
  ...
  const commonFilters = {
    page: page[activeQueue],
    limit: PAGE_SIZE,
    status: status[activeQueue],
    q: search,
  };
  const reviewsQuery = useAdminModerationReviewsList(
    activeQueue === "reviews"
      ? commonFilters
      : { page: page.reviews, limit: PAGE_SIZE, status: status.reviews },
  );
  const reportsQuery = useAdminModerationReviewReportsList(
    activeQueue === "reports"
      ? commonFilters
      : { page: page.reports, limit: PAGE_SIZE, status: status.reports },
  );
  const questionsQuery = useAdminModerationQuestionsList(
    activeQueue === "questions"
      ? commonFilters
      : { page: page.questions, limit: PAGE_SIZE, status: status.questions },
  );
  const answersQuery = useAdminModerationAnswersList(
    activeQueue === "answers"
      ? commonFilters
      : { page: page.answers, limit: PAGE_SIZE, status: status.answers },
  );
  ...
  const isNoteMissing = Boolean(
    actionTarget?.requiresNote && note.trim().length === 0,
  );

  const reviews = useMemo(
    () =>
      (reviewsQuery.data?.items ?? []).filter((item: AdminModerationReview) =>
        textMatch(
          [
            item.body,
            item.product.title,
  ...
  const reports = useMemo(
    () =>
      (reportsQuery.data?.items ?? []).filter(
        (item: AdminModerationReviewReport) =>
          textMatch(
            [
              item.reason,
              item.detail,
  ...
  const answers = useMemo(
    () =>
      (answersQuery.data?.items ?? []).filter((item: AdminModerationAnswer) =>
        textMatch(
          [
            item.answer,
            item.question.question,
            item.question.product.title,
            item.question.shop.name,
            item.user.email,
            item.status,
          ],
          search,
  ...
  function submitAction() {
    ...
    const options = {
      onSuccess: () => {
        setMessage(
          `${actionTarget.title} updated to ${actionTarget.status.replaceAll("_", " ")}.`,
        );
    ...
    if (actionTarget.queue === "questions")
      updateQuestion.mutate(payload, options);
    if (actionTarget.queue === "answers") updateAnswer.mutate(payload, options);
  }

  return (
    <AdminDataShell
      title={t("admin.pages.contentModeration.title")}
      description={t("admin.pages.contentModeration.description")}
      icon={MessageSquareWarningIcon}
      search={search}
      searchPlaceholder={t("admin.search.content")}
    ...
    >
      <CardContent className="p-0">
        ...
        <Tabs
          value={activeQueue}
          onValueChange={(value) => {
            setActiveQueue(value as QueueKey);
            setMessage(null);
          }}
          className="w-full"
        >
          <div className="border-b border-white/10 px-5 py-3">
            <TabsList className="bg-slate-950/60">
              {QUEUES.map((queue) => (
                <TabsTrigger key={queue} value={queue}>
                  {t(`admin.contentModeration.queues.${queue}`)}
                </TabsTrigger>
              ))}
            </TabsList>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-white/10 bg-white/6 hover:bg-white/6">
                  <TableHead className="px-5 text-slate-300">
                    {t("admin.ui.review")}
                  </TableHead>
                    ...
                    >
                      <TableCell className="max-w-md px-5 py-4">
                        <p className="font-medium text-white">
                          {t("admin.contentModeration.reviewSummary").replace("{rating}", String(item.rating)).replace("{count}", String(item.reportCount))}
                        </p>
                        <p className="mt-1 line-clamp-3 text-sm text-slate-300">
                          {item.body ?? t("admin.contentModeration.noReviewBody")}
                        </p>
                        ...
                        <StatusActions
                          ...
                          onSelect={(next) =>
                            openAction({
                              ...
                              status: next,
                              requiresNote: needsReviewNote(next),
                            })
                          }
                        />
                      </TableCell>
                  ...
                  reports.map((item) => (
                    ...
                    >
                      <TableCell className="max-w-md px-5 py-4">
                        ...
                        <p className="mt-1 line-clamp-3 text-sm text-slate-300">
                          {item.detail ?? t("admin.contentModeration.noDetail")}
                        </p>
                      </TableCell>
                      <TableCell>
                        <p className="text-sm text-slate-200">
                          {item.review?.product.title ?? t("admin.contentModeration.reviewRemoved")}
                        </p>
                        <p className="line-clamp-2 text-xs text-slate-500">
                          {item.review?.body ?? t("admin.contentModeration.noReviewBody")}
                        </p>
                      </TableCell>
                      <TableCell>
                        <p className="text-sm text-slate-200">
                        ...
                        <StatusActions
                          ...
                          onSelect={(next) =>
                            openAction({
                              ...
                              id: item.id,
                              title:
                                item.review?.product.title ?? t("admin.contentModeration.reviewReport"),
                              status: next,
                              requiresNote: needsReportNote(next),
                            })
                          }
                        />
                      </TableCell>
                  ...
                  questions.map((item) => (
                    ...
                    >
                      <TableCell className="max-w-md px-5 py-4">
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          {t("admin.contentModeration.answerCount").replace("{count}", String(item.answerCount))}
                        </p>
                      </TableCell>
                      <TableCell>
                        <p className="text-sm text-slate-200">
                        ...
                        <StatusActions
                          ...
                          onSelect={(next) =>
                            openAction({
                              ...
                              title: item.product.title,
                              status: next,
                              requiresNote: needsQuestionNote(next),
                            })
                          }
                        />
                      </TableCell>
                  ...
                  answers.map((item) => (
                    ...
                    >
                      ...
                      <TableCell>
                        ...
                        <p className="text-xs text-slate-500">
                          {t("admin.contentModeration.askedBy").replace("{email}", item.question.user.email)}
                        </p>
                      </TableCell>
                      <TableCell>
                        <p className="text-sm text-slate-200">
                        ...
                        <StatusActions
                          ...
                          onSelect={(next) =>
                            openAction({
                              queue: "answers",
                              id: item.id,
                              title: item.question.product.title,
                              status: next,
                              requiresNote: needsAnswerNote(next),
                            })
                          }
                        />
                      </TableCell>
        ...
        <AdminTablePagination
          ...
          total={activeQuery.data?.pagination.total ?? 0}
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
                {t("admin.ui.confirmModeration")}
              </AlertDialogTitle>
              <AlertDialogDescription>
                {t("admin.contentModeration.confirmDescription")
                  .replace(
                    "{title}",
                    actionTarget?.title ?? t("admin.contentModeration.content"),
                  )
                  .replace(
                    "{status}",
                    actionTarget
                      ? t(`admin.statuses.${actionTarget.status}`)
                      : t("admin.contentModeration.selectedStatus"),
                  )}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="space-y-2">
              <Label htmlFor="content-moderation-note">
                {t("admin.ui.moderationNote")}
              </Label>
              <Textarea
                ...
                placeholder={
                  actionTarget?.requiresNote
                    ? t("admin.contentModeration.requiredNote")
                    : t("admin.contentModeration.optionalNote")
                }
              />
              {isNoteMissing ? (
                <p className="text-sm font-medium text-red-600">
                  {t("admin.ui.moderationNoteRequired")}
                </p>
              ) : null}
            </div>
            <AlertDialogFooter>
              ...
              <AlertDialogAction asChild>
                <Button
                  disabled={!actionTarget || isNoteMissing || isMutating}
                  onClick={submitAction}
                >
                  {isMutating ? t("admin.ui.saving") : t("admin.ui.confirm")}
                </Button>
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
