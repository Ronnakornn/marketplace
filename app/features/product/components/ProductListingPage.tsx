...
export function ProductListingPage({
  ...
  const productsQuery = searchProductsQuery;
  ...
  useEffect(() => {
    const hasFilters = Boolean(brandId || attributeFilters || minPrice || maxPrice || rating || inStock || (sort && sort !== "relevance"));
    if (!hasFilters || (mode !== "search" && mode !== "category")) return;
    trackDiscoveryEvent({
      eventType: "filter_applied",
      query,
      source: "search_filters",
      filters: { categoryId, brandId, attributeFilters, minPrice, maxPrice, rating, inStock, sort },
      resultCount: products.length,
    });
  }, [mode, query, categoryId, brandId, attributeFilters, minPrice, maxPrice, rating, inStock, sort, products.length]);
  ...
  }), [listing?.facets, categoriesQuery.data, brandsQuery.data, query, categoryId, brandId, attributeFilters, minPrice, maxPrice, sort, rating, inStock, basePath, mode]);
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-6xl space-y-5 px-3 pb-28 pt-4">
        {mode === "home" ? <HomeBlocks categories={filterProps.categories.slice(0, 5)} /> : null}
        ...
        <div className={mode !== "home" ? "grid min-w-0 gap-4 lg:grid-cols-[260px_minmax(0,1fr)]" : ""}>
          ...
          <section className="min-w-0 space-y-3">
            <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs leading-5 text-slate-500">
                  {listing ? formatResultSummary(t("product.itemsFound"), t("product.itemsShown"), products.length, listing.meta.totalCount) : t("product.loadingResults")}
                </p>
              </div>
              {mode !== "home" ? (
                <SortTabs basePath={basePath} query={query} categoryId={mode === "search" ? categoryId : undefined} brandId={brandId} attributeFilters={attributeFilters} minPrice={minPrice} maxPrice={maxPrice} rating={rating} inStock={inStock} sort={sort} />
              ) : (
            ...
            {isInitialError ? <BuyerErrorState message={getReadableListingErrorMessage(productsQuery.error, t("state.loadErrorDescription"))} onRetry={() => void productsQuery.refetch()} /> : null}
            ...
            {products.length > 0 ? (
              <>
                ...
                <div className="flex flex-col items-center gap-2 py-3">
                  {isNextPageError ? (
                    <p className="text-sm text-red-600">{getReadableListingErrorMessage(productsQuery.error, t("state.loadErrorDescription"))}</p>
                  ) : null}
...
function HomeBlocks({ categories }: { categories: CategoryFilterOption[] }) {
  const t = useTranslations();
  const localePath = useLocalePath();
  return (
    <>
      <section className="rounded-3xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
        ...
        <div className="mt-4 grid grid-cols-5 gap-2">
          {categories.map((category) => (
            <Link key={category.slug} href={localePath(`/categories/${category.slug}`)} className="rounded-2xl border border-orange-100 bg-orange-50 px-2 py-3 text-center text-xs font-semibold text-orange-700 transition hover:bg-orange-100">
              {category.name}
            </Link>
          ))}
        </div>
    ...
    <div className="grid grid-cols-2 gap-2 lg:grid-cols-1">
      {sorts.map(([value, label]) => (
        <FilterLink
          ...
          href={buildSearchHref({ q: props.query, categoryId: props.categoryId, brandId: props.brandId, attributeFilters: props.attributeFilters, minPrice: props.minPrice, maxPrice: props.maxPrice, rating: props.rating, inStock: props.inStock, sort: value }, props.basePath)}
        >
          {label}
        </FilterLink>
      ))}
    ...
    <div className="flex max-w-full gap-1 overflow-x-auto rounded-full border border-slate-200 bg-white p-1 sm:max-w-[min(100%,34rem)]">
      {sorts.map(([value, label]) => (
        <Button
          ...
          asChild
        >
          <Link href={buildSearchHref({ q: props.query, categoryId: props.categoryId, brandId: props.brandId, attributeFilters: props.attributeFilters, minPrice: props.minPrice, maxPrice: props.maxPrice, rating: props.rating, inStock: props.inStock, sort: value }, props.basePath)}>{label}</Link>
        </Button>
      ))}
    </div>
  );
...
function formatResultSummary(template: string, shownTemplate: string, visibleCount: number, totalCount: number | null): string {
  ...
  return shownTemplate.replace("{visible}", String(visibleCount)).replace("{total}", String(totalCount));
}
...
function getReadableListingErrorMessage(error: unknown, fallback: string) {
