...
describe("product cart handoff", () => {
  ...
  it("normalizes readable error messages without object string output", () => {
    expect(normalizeAddToCartError("[object Object]")).toBe("Please try again or review the selected options.");
    expect(normalizeAddToCartError({ body: { message: "Variant is out of stock" } })).toBe("Variant is out of stock");
    expect(normalizeAddToCartError({ error: "Please sign in first" })).toBe("Please sign in first");
    expect(normalizeAddToCartError(null, "Retry add to cart")).toBe("Retry add to cart");
  });

  it("shows a readable add-to-cart error toast", () => {
    showAddToCartError({ error: { response: { error: "Network unavailable" } } });
