  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-5xl space-y-3 px-3 pb-28 pt-4">
        ...
        {favoritesQuery.isSuccess && favoritesQuery.data.length === 0 ? <BuyerEmptyState title="No favorites yet" description="Tap the heart on product pages to save items here." /> : null}
          ...
          <article key={favorite.id} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-2 font-bold text-orange-600">{formatMoney(favorite.priceCents, favorite.currency)}</p>
              </div>
              <HeartIcon className="size-5 fill-orange-500 text-orange-500" />
            </div>
            <Button asChild variant="outline" className="mt-3 rounded-full">
              <Link href={localePath(`/products/${favorite.productId}`)}>View product</Link>
            </Button>
          </article>
        ))}
      </div>
