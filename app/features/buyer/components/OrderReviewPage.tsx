...
export function OrderReviewPage({ orderId }: { orderId: string }) {
  ...
  return (
    <>
      <BuyerTopBar title="Review order" />
      <div className="mx-auto max-w-3xl space-y-4 px-3 pb-28 pt-4">
        ...
        {orderQuery.data && eligibleItems.length === 0 ? (
          <BuyerEmptyState title="No items ready for review" description="Reviews are available after the item is delivered." />
        ) : null}
        {eligibleItems.length > 0 ? (
          <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
            <h1 className="text-xl font-bold text-slate-950">Review delivered item</h1>
            ...
            <div className="mt-5 space-y-2">
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((value) => (
                  <button key={value} type="button" className="rounded-full p-1 text-amber-400" onClick={() => setRating(value)} aria-label={`${value} stars`}>
            ...
            <div className="mt-4 space-y-2">
              ...
              <Textarea id="review-comment" value={comment} onChange={(event) => setComment(event.target.value)} className="min-h-28 rounded-2xl" placeholder="Share product quality, shipping, and seller experience." />
            </div>
            {reviewMutation.isError ? <p className="mt-3 text-sm text-red-600">{reviewMutation.error.message}</p> : null}
            {reviewMutation.isSuccess ? <p className="mt-3 text-sm text-emerald-700">Review submitted.</p> : null}
          </section>
        ) : null}
