...
export function FollowedShopsPage() {
  ...
  return (
    <>
      <BuyerTopBar title="Followed shops" />
      <div className="mx-auto max-w-5xl space-y-3 px-3 pb-28 pt-4">
        ...
        {shopsQuery.isSuccess && shopsQuery.data.length === 0 ? <BuyerEmptyState title="No followed shops" description="Follow shops to see their products and promotions faster." /> : null}
          ...
          <article key={shop.id} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">{shop.followerCount} followers · {shop.productCount} products</p>
              </div>
              <Button asChild variant="outline" className="rounded-full"><Link href={localePath(`/shops/${shop.shopId}`)}>View shop</Link></Button>
            </div>
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-2">
                {shop.products.map((product) => (
                  <Link key={product.id} href={localePath(`/products/${product.id}`)} className="rounded-2xl border border-slate-100 bg-slate-50 p-3">
                    ...
                    <p className="mt-1 text-sm font-bold text-orange-600">{formatMoney(product.priceCents, product.currency)}</p>
                  </Link>
                ))}
              </div>
            ) : null}
